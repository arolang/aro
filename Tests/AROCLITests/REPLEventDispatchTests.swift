...
struct REPLEventDispatchTests {
    ...
    private struct Playground {
        ...
        /// name and saw each other's events, so `clear() drops handler
        /// subscriptions` failed whenever it drew a digit-heavy UUID
        /// and another test had already stored there. Draw from two
        /// whole UUIDs and keep a leading letter, so the token is
        /// never empty and effectively never repeats.
