#!/bin/bash
#
# ARO Plugin Developer's Guide - PDF Builder
...
OUTPUT_PDF="$OUTPUT_DIR/ARO-Plugin-Guide.pdf"
...
# Start with foreword if it exists
if [[ -f "$SCRIPT_DIR/Foreword.md" ]]; then
...
echo "Building ARO Plugin Developer's Guide..."
echo ""
...
# Fallback to HTML if no LaTeX
    ...
    HTML_OUTPUT="$OUTPUT_DIR/ARO-Plugin-Guide.html"
...
pandoc \
    ...
    --toc-depth=2 \
    --css="unix-style.css" \
    --metadata title="ARO: The Plugin Developer's Guide" \
    -f markdown+raw_html-yaml_metadata_block \
    --embed-resources \
    --self-contained \
    -o "$TEMP_HTML" \
