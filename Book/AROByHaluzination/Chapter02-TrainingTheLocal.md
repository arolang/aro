...
The fine-tuned model that ships with `aro lm` is not large. By the standards of 2026, it is small — a 4-bit quantised 30-billion-parameter coder with enough of its weights shaved off to run comfortably on a laptop with unified memory or a mid-range GPU. This was not a compromise. It was the point.
...
The training pipeline lives in the sibling project `ARO-Train`. It is a sequence of Jupyter notebooks — numbered `00` through `21` — that collect data, shape it into pairs, fine-tune a base model, and package the result. You do not need to run it to use `aro lm`. You only need to run it if you want to train your own variant.
...
3. **Generates instruction/response pairs** from the knowledge file, from the books, from the git history of the language, and from comments left in existing ARO applications. This is the bulk of the training data.
...
The fine-tuned model learned four things, in roughly decreasing order of how much pipeline time was spent on each:
...
**The four action roles.** REQUEST actions (Extract, Parse, Retrieve, Fetch) pull data from outside. OWN actions (Compute, Validate, Compare, Create, Transform) work inside the process. RESPONSE actions (Return, Throw) hand data back. EXPORT actions (Publish, Store, Log, Send, Emit) push data out. The model learned which verbs live in which category, and which prepositions each verb actually takes.
...
The fine-tune is published as `ARO-Lang/aro-coder-4bit` on Hugging Face. It is loaded automatically the first time you run `aro lm`. You do not need to run the training pipeline to use the model. You only need to install one of the supported runners — `llama-server`, `mlx_lm.server`, or any OpenAI-compatible endpoint — and the first `aro lm` invocation will offer to download the weights to `~/.cache/aro/models/`.

The interactive loop inside the training pipeline (notebook `21_chat.ipynb`) is the same loop that `aro lm` uses, in spirit: load the model, build a system prompt from the ARO knowledge base, pass user turns through the tokenizer, and stream back a reply. The difference is that `aro lm` does not require Python, does not require MLX, and does not require you to start Jupyter. It is the same brain in a simpler body.
