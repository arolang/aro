...
**Model selection.** The default `aro-coder-4bit` is a 4-bit quantised 30B model. If your machine struggles — if tokens come out slowly, if the fan spins up — try a smaller variant if one is available. A slightly less capable model that responds in two seconds is more useful than a slightly more capable model that takes thirty.
