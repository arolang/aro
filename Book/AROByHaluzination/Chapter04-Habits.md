...
The single most common mistake, made by everyone who picks up `aro lm` for the first time, is treating the context file as a long-lived project log. It is not. It is a scratchpad. The longer it gets, the worse the model gets.
...
The second most common mistake is pasting code into the prompt. Do not do this. The model has a tool — `read_file` — for precisely this reason. When you want the model to look at `Sources/AROParser/Lexer.swift`, do not paste the file. Say "read Sources/AROParser/Lexer.swift and summarise the tokenising strategy." The model will call `read_file` and report back.
...
When the model wants to run a shell command, `aro lm` stops and asks:
...
The first time you run `aro lm /index` in a project, it walks every indexable file, chunks them, embeds them, and writes the result to `.context.index/vectors.json`. After that, the model has access to a search tool that can find a relevant piece of the project in one call.
...
`aro lm` is excellent at writing ARO feature sets, explaining error messages, refactoring handlers, and chasing down compiler diagnostics. It is mediocre at writing commit messages, because the training data did not include many.
...
## 4.8 Shorter Prompts Are Better

The fine-tune likes short, direct prompts. "Write a feature set for `createUser` that validates the email and emits a `UserCreated` event" is better than "Could you please help me write a feature set that will create a user, and also make sure to validate that the email address they provide is in a valid format, and then after the user is created successfully, it would be good if we could emit an event saying the user was created..."

The reason is that the training data was terse. Every instruction/response pair in the fine-tune was short. The model learned that the shape of a request-to-`aro-coder` is: verb, subject, constraints, and nothing else. When you write in that shape, the model responds in kind. When you write long prompts, the model matches your style, and the reply gets longer and less focused.

There is a useful test: if you can remove a word from your prompt without changing the meaning, remove it. The model will thank you by being more correct.
