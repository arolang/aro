#!/bin/bash
#
# ARO by Haluzination - PDF Builder
...
OUTPUT_PDF="$OUTPUT_DIR/ARO-by-Haluzination.pdf"
...
echo "Building ARO by Haluzination..."
echo ""
...
# Fallback to HTML if no LaTeX
    ...
    HTML_OUTPUT="$OUTPUT_DIR/ARO-by-Haluzination.html"
...
pandoc \
    ...
    --toc-depth=2 \
    --css="unix-style.css" \
    --metadata title="ARO by Haluzination" \
    -f markdown+raw_html-yaml_metadata_block \
    --embed-resources \
    --self-contained \
    -o "$TEMP_HTML" \
