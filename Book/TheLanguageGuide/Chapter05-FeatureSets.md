...
Feature sets execute in response to events. The runtime maintains an event bus that routes events to matching feature sets based on their headers. Understanding which patterns match which events is essential for building applications that behave as intended.

The application lifecycle provides three reserved triggering patterns. The name `Application-Start` triggers when the application launches. This is the entry point, equivalent to `main` in other languages. Every application must have exactly one feature set with this name. Having none means there is nothing to execute. Having multiple means there is ambiguity about where to begin.

The name `Application-End` with business activity `Success` triggers during graceful shutdown. This is where you close connections, flush buffers, and perform cleanup. It is optional—many applications need no cleanup. The name `Application-End` with business activity `Error` triggers when the application encounters an unrecoverable error. This is where you log the failure, notify administrators, or attempt recovery. It too is optional.

For applications with HTTP APIs, feature set names match operation identifiers from the OpenAPI specification. When a request arrives at a path defined in the specification, the runtime looks up the operation identifier for that path and method combination, then triggers the feature set with that name. This connection between specification and implementation is what makes ARO contract-first: the API contract drives which feature sets handle which requests.

Event handlers follow a naming convention. When a business activity ends with "Handler" and is preceded by an event name, the feature set becomes a handler for that event type. The pattern is flexible: `UserCreated Handler`, `File Event Handler`, `PaymentFailed Handler`. When an event of the matching type is emitted, all handlers for that event type execute.

Multiple handlers can exist for the same event. When you emit a `UserCreated` event, all feature sets whose business activity matches `UserCreated Handler` will execute. The order of execution is not guaranteed. This allows you to add behaviors by adding feature sets without modifying existing code—a form of the open-closed principle expressed through event-driven architecture.
