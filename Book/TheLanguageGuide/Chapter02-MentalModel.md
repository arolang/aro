...
The fourth role is called **EXPORT**, which describes actions that make data globally available without terminating execution. Export actions broadcast data to other feature sets. Verbs like Emit, Publish, and Schedule carry the export role. (Note: Store, Log, and Send are classified as RESPONSE — they produce side effects but do not terminate execution.)

The fifth role is called **SERVER**, which manages service lifecycle. Starting an HTTP server, stopping a service, listening for keyboard input, or keeping the application alive are server operations. They establish or tear down infrastructure without directly moving business data.
<div style="clear: both;"></div>
Understanding these roles helps you reason about your programs. A typical feature set begins with request actions that gather the needed data, follows with own actions that process and transform that data, uses response actions to persist results or notify other parts of the system, and concludes with a return that sends the final result to the caller. This pattern emerges naturally from the semantic roles.
