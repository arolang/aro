...
Within a feature set, statements execute in order from top to bottom. There is no branching and no early return. Execution begins with the first statement and proceeds through each subsequent statement until reaching the end or encountering an error. The one construct that departs from a straight line is `for each`, which runs its block once per element of a collection (Chapter 33); it still has a single entry and a single exit, so the statement after the loop is the statement after the loop.
