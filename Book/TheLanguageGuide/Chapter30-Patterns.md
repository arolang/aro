...
## 30.5 Command Query Responsibility Segregation
...
Use events for side effects and communication. Rather than calling between feature sets, emit events and let handlers react. This decoupling makes the system more flexible and easier to evolve. Where the thing you want really is a call — a reusable computation whose result you need right here — a feature set whose business activity is `Action` becomes callable as `Application.<Name>` (ARO-0081, Chapter 24), and that is clearer than routing a synchronous need through the event bus.
...
*Next: Chapter 31 — State Machines*
