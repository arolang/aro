...
A hyphenated name may contain any word, reserved or not. `with-tax`, `tax-with`, `content-type`, `by-age` and `from-date` all parse, alongside `taxed-price`, `youngest-first` and `user-email-address`. A word that begins a statement or introduces an object elsewhere is just a word once it is part of a name, because a name is not a place where a statement can start. What a hyphen may not be followed by is something that is not a word at all: `<a-42>` is an error, because a number does not extend a name.
The same freedom covers the segments of a qualifier path, where you do not get to choose the name in the first place. `<request: headers.Content-Type>` reads the header as written. Quoting a segment — `<headers: "Content-Type">` — remains available and matches it as a plain key (Chapter 10), which is what you want for a key containing a space or a dot.
