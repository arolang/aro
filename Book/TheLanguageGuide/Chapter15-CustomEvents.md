...
The emitter does wait for every matching handler to finish before continuing past the `Emit` statement, so the next statement can rely on side effects the handlers performed (state mutation, repository writes, log lines). What it does not do is check whether handlers succeeded—each handler's outcome stays isolated, which is what prevents cascading failures.
...
## 15.9 Typed Event Extraction (ARO-0046)
...
## 15.11 Debugging with Event Recording
...
See Chapter 13.12 for detailed information about event recording and replay.

---
*Next: Chapter 16 — Handler Guards*