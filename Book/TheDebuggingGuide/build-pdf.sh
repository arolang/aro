#!/bin/bash
#
# ARO Debugging Guide - PDF Builder
...
OUTPUT_PDF="$OUTPUT_DIR/ARO-Debugging-Guide.pdf"
...
echo "Building ARO Debugging Guide..."
echo ""
...
# Fallback to HTML if no LaTeX
    ...
    HTML_OUTPUT="$OUTPUT_DIR/ARO-Debugging-Guide.html"
...
pandoc \
    ...
    --toc-depth=2 \
    --css="unix-style.css" \
    --metadata title="ARO: Debugging Guide" \
    -f markdown+raw_html-yaml_metadata_block \
    --embed-resources \
    --self-contained \
    -o "$TEMP_HTML" \
