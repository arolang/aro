...
The entire pipeline was written by the same method as the rest of ARO: a person asking a general model what each step should look like, reading the draft, running it, fixing it, and asking for the next step. The training script that produced `aro-coder-6bit` was itself the product of a long conversation with a model that did not know what ARO was, and that needed to be told.
