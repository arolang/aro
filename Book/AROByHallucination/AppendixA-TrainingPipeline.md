...
The notebooks are numbered by filename, and `00_META_PIPELINE.ipynb` runs them in the order below, each in its own isolated kernel. Filename, heading and table now agree: the numbering is derived from the meta notebook's own list by `Scripts/generate-train-stages.py`, which CI runs, so a renumber that leaves one of them behind fails the build rather than reaching a reader.
