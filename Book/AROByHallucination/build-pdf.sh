#!/bin/bash
#
# ARO by Hallucination - PDF Builder
...
OUTPUT_PDF="$OUTPUT_DIR/ARO-by-Hallucination.pdf"
...
echo "Building ARO by Hallucination..."
echo ""
...
# Fallback to HTML if no LaTeX
    ...
    HTML_OUTPUT="$OUTPUT_DIR/ARO-by-Hallucination.html"
...
pandoc \
    ...
    --toc-depth=2 \
    --css="unix-style.css" \
    --metadata title="ARO by Hallucination" \
    -f markdown+raw_html-yaml_metadata_block \
    --embed-resources \
    --self-contained \
    -o "$TEMP_HTML" \
