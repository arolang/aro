...
`aro ask` is a small, specific bet on that reversal. It is not a general-purpose coding assistant. It is a coding assistant for one language, running locally, with tools that are scoped to one project. It does not phone home. It does not require an account. It does not need network access once the model is downloaded. It does not train on your conversations.
